...
def main(argv):
    ...
    path_to_metadata = flags.FLAGS.path_to_metadata
    if not flags.FLAGS.path_to_metadata:
        path_to_metadata = "/tmp/mediapipe/metadata.pb"
    metadata = tf.train.SequenceExample()
    ms.set_clip_data_path(bytes23(flags.FLAGS.path_to_input_video), metadata)
    ms.set_clip_start_timestamp(flags.FLAGS.clip_start_time_sec * SECONDS_TO_MICROSECONDS, metadata)
    ms.set_clip_end_timestamp(flags.FLAGS.clip_end_time_sec * SECONDS_TO_MICROSECONDS, metadata)
    with open(path_to_metadata, 'wb') as writer:
...
if __name__ == '__main__':
    ...
    flags.DEFINE_string('path_to_metadata', '', 'Path to the output metadata.')
    flags.DEFINE_integer('clip_start_time_sec', 0, 'Clip start timestamp in seconds')
